...
export class MessageService {
  ...
  createHubConnection(otherUserId: string){
    ...
    this.hubConnection.on("ReciveMessageThread", (messages: Message[]) => {
      this.messageThread.set(messages.map(message => ({...message, currentUserSender: message.senderId !== otherUserId})))
    })
  ...
  sendMessage(recipientId: string, content: string){
    return this.hubConnection?.invoke<Message>(`SendMessage`, {recipientId: recipientId, content: content})
  }
