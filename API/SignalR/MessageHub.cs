...
public class MessageHub(IUnitOfWork uow, IHubContext<PresenceHub> presenceHub) : Hub
{
    ...
    {
        var sender = await uow.MemberRepository.GetMemberById(GetUserId());
        var recipient = await uow.MemberRepository.GetMemberById(createMessageDto.RecipientId);
    ...
    {
        return Context.User?.GetMemberId() ?? throw new HubException("Connot get member id");
    }
}
